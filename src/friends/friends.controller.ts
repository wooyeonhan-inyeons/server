...
export class FriendsController {
    ...
    @Post()
    @ApiCreatedResponse({ status: 200, type: ResponseCreateFriendDto, description: "친구 신청을 합니다."})
    @Roles([Role.User])
    ...
    @Get()
    @ApiCreatedResponse({ status: 200, type: ResponseGetAllFriendDto, description: "모든 친구를 조회합니다."})
    @Roles([Role.User])
    ...
    async acceptFriendRequest(@Req() req, @Body() body: RequestAcceptFriendDto){
        return await this.friendsService.acceptFriendRequest(req.user.user_id, body.friend_id)
    }

    @Post("/decline")
    @Roles([Role.User])
    async declineFriendRequest(@Req() req, @Body() body: RequestDeclineFriendDto){
        return await this.friendsService.declineFriendRequest(req.user.user_id, body.friend_id)
    }

    @Get("/request")
    @ApiCreatedResponse({ status: 200, type: ResponseGetFriendRequestDto, description: "받은 친구요청 목록을 조회합니다.", isArray: true})
    @Roles([Role.User])
    ...
    @Get("/requested")
    @ApiCreatedResponse({ status: 200, type: ResponseGetRequestFriendListDto, description: "보낸 친구요청 목록을 조회합니다.", isArray: true})
    @Roles([Role.User])
    ...
    async deleteFriend(@Req() req, @Body() body: RequestDeleteFriendDto){
        return await this.friendsService.deleteFriend(req.user.user_id, body.friend_id)
    }
}
