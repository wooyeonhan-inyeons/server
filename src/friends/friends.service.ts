...
export class FriendsService {
    constructor(
      ...
      async createRelation(follower_uuid: string, following_uuid: string){
        ...
        if(follower == null || following == null) throw new BadRequestException({
          status: HttpStatus.BAD_REQUEST,
          message: "유저가 존재하지 않습니다."
        })

        const isRequested = await this.friendsRepository.createQueryBuilder("friend")
        ...
        if(isRequested != null) throw new BadRequestException({
          status: HttpStatus.BAD_REQUEST,
          message: "이미 신청을 보냈습니다."
        })

        const isFriend = await this.friendsRepository.createQueryBuilder("friend")
        .where("friend.follower = :follower_uuid", {follower_uuid})
        .andWhere("friend.following = :following_uuid", {following_uuid})
        .andWhere("friend.relation_type = 1")
        .getOne();
        
        if(isFriend != null) throw new BadRequestException({
          status: HttpStatus.BAD_REQUEST,
          message: "이미 친구 입니다.",
        })
      ...
      async acceptFriendRequest(user_id: string, friend_id: string){
        ...
        //신청받은 사람이 아니면 못받음
        if(friend_request.following?.user_id != user_id) throw new BadRequestException({
          status: HttpStatus.BAD_REQUEST,
          message: "잘못된 요청입니다.",
        })

        if(friend_request?.relation_type == 1) throw new BadRequestException({
          status: HttpStatus.BAD_REQUEST,
          message: "이미 친구입니다.",
        })
